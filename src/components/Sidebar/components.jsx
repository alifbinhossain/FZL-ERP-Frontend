...
const SingleMenu = ({ items }) => {
	const cls = `block  px-2 py-[5px] duration-500`;
	return (
		<ul className='sb-red text-md text-primary-content transition-all duration-500 ease-in-out'>
			{items.map((item) => (
				<NavLink
					to={item.path}
					key={item.path}
					className={(nav) =>
						nav.isActive
							? `thick-border-active rounded text-secondary-content ${cls}`
